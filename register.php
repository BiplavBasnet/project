...
if(isset($_POST['submit'])){
   ...
   if(!preg_match('/^(?=.*[!@#$%^&*(),.?":{}|<>])(?=.*[0-9])(?=.*[A-Z])(?=.*[a-z]).{8,}$/', $_POST['password'])){
      $message[] = 'Password must be at least 8 characters long, include at least one special character, one number, one uppercase, and one lowercase letter.';
   } else {
   ...
   <script>
      function validatePassword(){
         ...
         if(!regex.test(password)){
            alert('Password must be at least 8 characters long, include at least one special character, one number, one uppercase, and one lowercase letter.');
            return false;
         }
         return true;
      }
